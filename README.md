...
프로젝트 디렉터리는 **파일의 역할을 기준으로 분리**하였다.
...
각 파일 및 디렉터리의 역할은 다음과 같다.

* `README.md`

  * 프로젝트 전체 수행 과정, 실행 방법, 검증 결과 및 트러블슈팅 내용을 정리한 기술 문서이다.
* `Dockerfile`

  * `nginx:alpine`을 기반으로 웹 서버 이미지를 만들기 위한 설정 파일이다.
* `index.html`

  * Docker 컨테이너에서 NGINX를 통해 제공할 실제 웹 페이지 파일이다.
* `terminal-practice/`

  * 파일 및 디렉터리 생성, 복사, 이동, 삭제 등의 터미널 실습 결과를 분리하여 저장하였다.
* `bind-site/`

  * Bind Mount 실습에 사용하는 웹 콘텐츠를 별도로 관리하기 위해 구성하였다.
* `evidence/`

  * 터미널 실행 결과, Docker 실행 결과, 웹 브라우저 접속 결과 등 과제 수행 증거 스크린샷을 모아두었다.

즉, **실제 실행에 필요한 파일, 기능별 실습 파일, 증거 자료를 역할별로 분리하여 프로젝트 구조를 쉽게 파악할 수 있도록 구성하였다.**

이를 통해 다른 사람이 Repository를 확인할 때 각 파일의 역할을 쉽게 이해할 수 있고, 수행 과정과 결과도 빠르게 확인할 수 있도록 하였다.

---

포트 및 Volume 설정의 재현 가능성

포트와 Docker Volume 설정은 다른 사람이 README만 보고도 동일한 환경을 구성할 수 있도록 **실제 사용한 명령어, 고정된 값, 검증 방법을 함께 기록**하였다.

### 포트 설정

웹 서버 컨테이너는 다음과 같이 실행하였다.
...
따라서 다른 사용자가 동일한 명령을 실행한 뒤

```text
http://localhost:8080
```

으로 접속하면 같은 웹 서버 결과를 확인할 수 있다.

포트 연결 상태는 다음 명령으로 확인하였다.

```bash
docker ps
```

결과에서 다음과 같은 포트 매핑을 확인하였다.

```text
0.0.0.0:8080->80/tcp
```

즉, 단순히 "웹 서버를 실행했다"고 기록한 것이 아니라 **실행 명령 → 포트 설정 → 상태 확인 → 브라우저 접속**까지 재현 가능한 형태로 기록하였다.

### Docker Volume 설정

Volume은 다음과 같이 이름을 고정하여 생성하였다.
...
첫 번째 컨테이너에서 `/data/hello.txt`에 데이터를 저장한 뒤 컨테이너를 삭제하였다.

```bash
docker rm -f week1-vol-1
```

이후 새로운 컨테이너에 동일한 Volume을 다시 연결하였다.
...
그리고 다음 명령으로 기존 데이터를 확인하였다.

```bash
docker exec week1-vol-2 cat /data/hello.txt
```
...
이를 통해 컨테이너를 삭제해도 동일한 Volume을 다시 연결하면 기존 데이터가 유지되는 것을 확인하였다.

따라서 포트와 Volume 모두 **이름과 연결 값을 명확하게 지정하고, 실제 실행 명령과 검증 절차까지 README에 기록하여 동일한 환경을 재현할 수 있도록 구성하였다.**
